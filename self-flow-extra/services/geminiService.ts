...
import { GroundingChunk, ScrapedContactInfo } from '../types';
        ...
        const response: GenerateContentResponse = await withRetry(() => ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: Type.OBJECT,
                    properties: {
                        url: { type: Type.STRING, description: "The final, canonical URL of the company's website that was analyzed." },
                        emails: {
                            type: Type.ARRAY,
                        ...
                        people: {
                            ...
                            items: {
                                ...
                                properties: {
                                    ...
                                    role: { type: Type.STRING, description: "Role or title." },
...
export const scrapeContactInfoFromUrls = async (chunks: GroundingChunk[], onProgress: (scrapedItem: ScrapedContactInfo) => void): Promise<void> => {
  for (const chunk of chunks) {
    try {
      ...
      // A longer delay helps ensure we stay within the free tier limits (e.g., 60 requests per minute).
      await sleep(2000); 
    } catch (error) {
