...
import { Campaign, AIStyleSettings, Tone, SalesApproach, ResponseLength, Post, CommentVariations } from '../types';
...
const AiResponseGeneratorModal: React.FC<AiResponseGeneratorModalProps> = (props) => {
    const { post, campaign, onClose, onUseComment, savedAiStyle, onSaveStyle, onGenerateAiResponse } = props;
    ...
    const [previews, setPreviews] = useState<CommentVariations | null>(null);
    const [activeTab, setActiveTab] = useState<'helpful' | 'questioning' | 'direct'>('helpful');
    ...
    useEffect(() => {
        // Automatically generate a preview when the modal opens
        handleGenerate();
    }, []);

    const handleGenerate = async () => {
        const canGenerate = onGenerateAiResponse();
        if (!canGenerate) {
        ...
        setPreviews(null); // Clear previous previews
        ...
        try {
            const response = await generateComment(post, campaign, currentSettings);
            setPreviews(response);
            setActiveTab('helpful'); // Default to helpful tab
        } catch (error) {
            console.error("Failed to generate comment variations", error);
        } finally {
            setIsGenerating(false);
        }
    };

    const tabKeys: Array<keyof CommentVariations> = ['helpful', 'questioning', 'direct'];
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-40 p-4" onClick={onClose}>
            <div className="bg-[var(--bg-secondary)] w-full max-w-lg rounded-2xl p-6 sm:p-8 border border-[var(--border-color)] shadow-2xl relative flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-y-auto pr-2 -mr-2 space-y-6">
                    ...
                    {(isGenerating || previews) && (
                        <div className="space-y-2 animate-fade-in">
                            <label className="block text-sm font-medium text-[var(--text-secondary)]">Previews</label>
                             <div className="w-full bg-[var(--bg-tertiary)] border border-gray-600 rounded-lg text-sm min-h-[160px]">
                                {isGenerating ? (
                                    <div className="flex items-center justify-center h-full text-[var(--text-secondary)] p-4">
                                        ...
                                        <div className="flex border-b border-gray-600">
                                            {tabKeys.map(key => (
                                                <button key={key}
                                                    ...
                                                    className={`flex-1 p-3 text-sm font-semibold capitalize transition-colors ${activeTab === key ? 'text-[var(--brand-primary)] bg-violet-500/10' : 'text-gray-400 hover:bg-white/5'}`}
                                                >
                                                    {key}
                                                </button>
                                            ))}
                                        </div>
                                        <div className="p-4 text-[var(--text-primary)] whitespace-pre-wrap">{previews[activeTab]}</div>
                                         <div className="p-4 border-t border-gray-600">
                                            <button onClick={() => onUseComment(previews[activeTab])} className="w-full bg-gradient-to-r from-[var(--brand-primary)] to-[var(--brand-secondary)] text-white font-semibold px-5 py-2.5 rounded-lg shadow-md hover:opacity-90 transition-opacity">
                                                Use This Comment
                                            </button>
                                        </div>
                                    </div>
                ...
                <div className="flex flex-col sm:flex-row justify-end items-center gap-4 mt-6 pt-6 border-t border-[var(--border-color)]">
                    <button onClick={handleGenerate} disabled={isGenerating} className="flex items-center justify-center gap-2 bg-black/20 dark:bg-white/10 text-[var(--text-secondary)] font-semibold px-5 py-2.5 rounded-lg hover:bg-black/30 dark:hover:bg-white/20 transition-colors w-full sm:w-auto disabled:opacity-50">
