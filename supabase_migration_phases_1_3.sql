...
CREATE TABLE IF NOT EXISTS subreddit_rules (
    ...
    rules JSONB, -- Array of rule objects {title, description, kind}
...
-- RLS Policies for discovered_subreddits
CREATE POLICY "Users can view their own discovered subreddits"
    ON discovered_subreddits FOR SELECT
    USING (
        campaign_id IN (
...
CREATE POLICY "Users can insert their own discovered subreddits"
    ON discovered_subreddits FOR INSERT
    WITH CHECK (
        campaign_id IN (
            SELECT id FROM campaigns WHERE user_id = auth.uid()::text
        )
...
CREATE POLICY "Users can update their own discovered subreddits"
    ON discovered_subreddits FOR UPDATE
    USING (
        campaign_id IN (
            SELECT id FROM campaigns WHERE user_id = auth.uid()::text
        )
...
CREATE POLICY "Users can delete their own discovered subreddits"
    ON discovered_subreddits FOR DELETE
    USING (
        campaign_id IN (
            SELECT id FROM campaigns WHERE user_id = auth.uid()::text
        )
...
-- RLS Policies for subreddit_rules (public read, admin write)
CREATE POLICY "Anyone can view subreddit rules"
    ON subreddit_rules FOR SELECT
    USING (true);

CREATE POLICY "System can insert subreddit rules"
    ON subreddit_rules FOR INSERT
    WITH CHECK (true);

CREATE POLICY "System can update subreddit rules"
    ON subreddit_rules FOR UPDATE
    USING (true);
